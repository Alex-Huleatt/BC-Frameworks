 ...
 * @author Alex
 */
public class SimplePather extends Mover {
    ...
    public SimplePather(RobotController rc, MapLocation goal) {
        super(rc,goal);
    ...
    private void check(MapLocation parent, int dir) throws GameActionException {
        ...
        if (impassable(unOffsetMapLocation(n))) {
            return;
        }
        final int nx = n.x;
        final int ny = n.y;
        ...
        if (prev[nx][ny] == 0 || cost[nx][ny] > potentialCost) {
            add(n, n.distanceSquaredTo(dest) + potentialCost); //* ((rc.senseTerrainTile(n)==TerrainTile.UNKNOWN)?5f:1f)
            prev[nx][ny] = dir;
